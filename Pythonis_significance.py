...
def fullKOOAStudy(working_directory, genes_list_file, network_structure_file, nb_columns_genes=2, name_index=0,
    ...
    gene_list, network, unused_genes = ImportBooleanModel(working_directory, genes_list_file, network_structure_file, nb_columns_genes, name_index, nb_columns_network, network_headers)
    ...
    for (param1, param2) in parameter_list:
        ...
        print '# Running model set : ', param1, param2
        print '#'
        ...
        for starting_point in starting_states_set:
            ...
            if (len(starting_states_set) > 10):
                print "# Running - %.0f%% of all runs done" % (100 * float(runcount) / float(len(starting_states_set)))
            else:
                print "# Running - run", runcount, "out of", len(starting_states_set)

            flow, stable, start, stable_states, initial_states, genes_names, data, networkoutput, runtime = RunBooleanModel(gene_list, network, 't', 't',
                                         initial_state_number='specified', initial_state_choice='random',
                                          stimulus=param2, initial_state_genes=['foo'], model=param1, specified_starting_states = [starting_point], verbose = verbose_output)
            ...
            for gene in gene_list:
                ...
                if (len(gene_list) > 10):
                    if not (genecount % tenpercentgene):
                        print "# Mutating - %.0f%% of all genes done" % (100 * float(genecount) / float(len(gene_list)))
                else:
                    print "# Mutating - gene", genecount, "out of", len(gene_list)

                # run all possible mutated network for gene KO or overexpressor
                flow, stable_KO, start, stable_states, initial_states, genes_names, data, networkoutput, runtime = RunBooleanModel(gene_list, network, 't', 't',
                    initial_state_number='specified', initial_state_choice='random',
                ...
                flow, stable_OA, start, stable_states, initial_states, genes_names, data, networkoutput, runtime = RunBooleanModel(gene_list, network, 't', 't',
                    initial_state_number='specified', initial_state_choice='random',
                    stimulus=param2, initial_state_genes=['foo'], model=param1,
    ...
    for k, v in meansKO.items():
        ...
        print '# gene :', k, '- mean KO distance :', meansKO[k],'- mean OA distance :', meansOA[k],' - over',total_nb_runs, 'run combinations.'

    end_time = time.time() - start_time
    print '# Running time (seconds) %s', end_time
    print '#\n#########################################'
...
def targetedKOStudy(working_directory, genes_list_file, network_structure_file, nb_columns_genes=2, name_index=0,
            ...
            for KO_set in KO_genes_list:
                ...
                if (len(KO_genes_list) > 10):
                    if not (genecount % tenpercentmutants):
                        print "# Mutating - %.0f%% of all KO set done" % (100 * float(genecount) / float(len(KO_genes_list)))
                else:
                    print "# Mutating - KO set", genecount, "out of", len(KO_genes_list)
    ...
    for k, v in meansKO.items():
        total_nb_runs = len(parameter_list)*N
        meansKO[k] = meansKO[k] / total_nb_runs
        print '# gene :', k, '- mean KO distance :', meansKO[k],' - over',total_nb_runs, 'run combinations.'
...
if __name__ == "__main__":
    ...
    rice_filenames = ['ReseauConsolideLitterature.sif','unionnetwork.sif', 'liste215_sansprior_moy3rep.sif'] #rice run - output in C:\Pythonis\Files_to_run\significance and gene list genes_list_os.txt
    ...
    for filename in arabido_filenames :
        ...
        gene_list, output, meansKO = targetedKOStudy("C:\Work\Papers\PYTHONIS\LRPnetwork-201903".decode("utf-8"), "l_gnp-221018-161genes-Ju+PI-prior.txt", filename, nb_columns_genes=1, name_index=1, nb_columns_network=3, starting_sample_size=sample_size,KO_genes_list=PLTARFKO)
